 ...
 * description: the main data gatherer, running in the background of the chrome window;
 * contains functions that are called when the tab is activated in order to get proper
 * information such as tab url and timesstamp upon opening. Additional functions include
 * updating the storage database with relevant information: whitelist, current tab, work time,
 * play time, etc, so that these items may be reachable in other javascript files. */
...
chrome.runtime.onInstalled.addListener(function() {
 ...
 /**********************************************************************************
                                   TIMESTAMP FUNCTIONS
                            originally from another github repo
                    NOT OURS BUT LOWKEY FORGOT WHOSE IT WAS ALL CREDITS DUE
 ***********************************************************************************/
 ...
 function sort() {
   chrome.storage.local.get(["whitelist"], function(whitelist) {
     chrome.storage.local.get(["history"], function(history) {
       ...
       //reset the workTime and playTime to 00:00:00 in order to add correctly
         ...
         chrome.storage.local.remove(["playTime"], function() {
           chrome.storage.local.set({"workTime":"00:00:00"}, function() {
             chrome.storage.local.set({"playTime":"00:00:00"}, function() {
               //for each url, check whether in whitelist or not
               historyArray.forEach(function(url) {
                 chrome.storage.local.get([url], function(data) {
                   //if in whitelist, add times to work time
                   if (whitelistArray.includes(url) == true) {
                     ...
                     //if not, add times for play time
          ...
          if (info.status === 'complete') {
            ...
            if (domain == "newtab") {
              ...
              var time = timeStamp();
                ...
                //check the page, update the visited pages history, and update current tab
